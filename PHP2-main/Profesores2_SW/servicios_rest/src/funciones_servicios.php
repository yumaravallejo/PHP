...
function obtener_guardias($dia)
{
    ...
    // Consulta
    try {
        $consulta = "select usuarios.id_usuario, usuarios.nombre, dia, hora from usuarios,horario_lectivo,grupos  where id_usuario=horario_lectivo.usuario && grupo=id_grupo && grupos.nombre = 'GUARD' && dia = ?";
        $sentencia = $conexion->prepare($consulta);
