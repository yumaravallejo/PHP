...
if (isset($_POST["btnContEditar"])) {
    ...
    if (!$error_form) {
        ...
        $datos = array("api_key" => $_SESSION["api_key"], "nombre" => $_POST["nombre"], "usuario" => $_POST["usuario"], "clave" => md5($_POST["clave"]), "email" => $_POST["email"]);
...
if (isset($_POST["btnContBorrar"])) {
    ...
    $respuesta = consumir_servicios_REST($url, "DELETE", array("api_key" => $_SESSION["api_key"]));
    $obj = json_decode($respuesta);
    var_dump($obj);
    if (!$obj) {
        session_destroy();
