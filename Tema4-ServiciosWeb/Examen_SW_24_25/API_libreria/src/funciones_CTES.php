...
function repetido_editar($tabla, $columna, $valor, $columna_key, $valor_key) {
    try{
        $conexion=new PDO("mysql:host=".SERVIDOR_BD.";dbname=".NOMBRE_BD,USUARIO_BD,CLAVE_BD,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
    }
    catch(PDOException $e)
    ...
    try{
        $consulta="SELECT ".$columna." FROM ".$tabla." WHERE ".$columna."=? AND ".$columna_key."<>?";
        $sentencia=$conexion->prepare($consulta);
        $sentencia->execute([$valor, $valor_key]);
    } catch(PDOException $e)
    {
        $sentencia=null;
        $conexion=null;
