...
if(isset($_POST["btnContNuevo"]))
{
    ...
    if(!$error_cod)
    {
        ...
        $url=DIR_SERV_ADMIN."/repetido/producto/cod/".urlencode($_POST["cod"]);
    ...
    {
        $url=DIR_SERV_ADMIN."/repetido/producto/nombre_corto/".urlencode($_POST["nombre_corto"]);
        $respuesta=consumir_servicios_REST($url,"GET");
        $json_repetido=json_decode($respuesta,true);
        if(!$json_repetido){
            session_destroy();
    ...
    if(!$error_form)
    {
        ...
        $url=DIR_SERV_ADMIN."/producto/insertar";
...
{
    $url=DIR_SERV_ADMIN."/producto/".urlencode($_POST["btnDetalles"]);
    $respuesta=consumir_servicios_REST($url,"GET");
...
{
    $url=DIR_SERV_ADMIN."/producto/borrar/".urlencode($_POST["btnContBorrar"]);
...
<body>
    ...
    <h2 class="centrado txt_centrado">Listado de los productos</h2>
    <?php
