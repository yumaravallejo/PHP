...
$(function () {
  ...
  } else cargar_vista_login("");
});

function cargar_vista_oportuna(datos_usu_log) {
  $("#errores").html("");
  let html_principal =
    "<div id='saludo'>Bienvenido <strong>" +
    datos_usu_log["lector"] +
    "</strong> - <button onclick='cerrar_sesion();' class='enlace'>Salir</button></div>";
  if (datos_usu_log["tipo"] == "admin") {
    html_principal +=
      "<h2>Listado de los Libros</h2><div id='libros'></div><div id='respuestas'></div>";
    $("#principal").html(html_principal);
    cargar_libros_admin();
    cargar_formulario_agregar();
  } else {
    html_principal +=
      "<h2>Listado de los Libros</h2><div class='contenedor_libros'></div>";
    $("#principal").html(html_principal);
...
function cargar_vista_login(error) {
  ...
  html_form_login +=
    "<p><label for='usuario'>Usuario: </label><input type='text' id='usuario' required/><span id='error_login' class='error'></span></p>";
  html_form_login +=
    "<p><label for='clave'>Clave: </label><input type='password' id='clave' required/></p>";
  ...
  html_form_login += "<h2>Listado de los Libros</h2>";
  html_form_login += "<div class='contenedor_libros'></div>";
